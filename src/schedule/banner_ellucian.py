...
class BannerEllucianProvider:
    ...
    def search_course(
        ...
        if not self.supports_source(source):
            raise ValueError(
                f"Provider BannerEllucianProvider does not support source system="
...
def _parse_section(
    ...
    title = (
        _first_str(body.get("Title"), body.get("SectionTitleDisplay"), body.get("CourseName"))
        or ""
    )
...
def _normalize_modality(*, instructional_methods: object, meetings: object) -> str:
    ...
    if methods_text:
        if "hybrid" in methods_text:
            return "hybrid"
        if (
            "online" in methods_text
            or "asynchronous" in methods_text
            or "synchronous" in methods_text
        ):
            return "online"
